...
# The CLI
# --------------------------------------------------------------------------
def write_manifest(tmp_path, source: SourceDocument) -> str:
...
def test_the_extract_command_reports_what_it_found(source, tmp_path, capsys) -> None:
    assert cli_main(["extract", "--sources", write_manifest(tmp_path, source)]) == 0
...
def test_the_missing_source_manifest_explains_itself(tmp_path) -> None:
    with pytest.raises(SystemExit) as excinfo:
        cli_main(["extract", "--sources", str(tmp_path / "absent.json")])
    assert "Create one listing the documents" in str(excinfo.value)
